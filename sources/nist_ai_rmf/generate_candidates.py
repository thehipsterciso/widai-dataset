#!/usr/bin/env python3
"""
Generate bi-encoder candidate identifications for NIST AI RMF elements.

For each subcategory outcome element, encodes the competency_implication text
and finds the top-5 most similar WIDAI KSAs by cosine similarity using
all-MiniLM-L6-v2.

Output: nist_ai_rmf_candidates.json
"""
...
ELEMENTS_FILE = os.path.join(os.path.dirname(__file__), "nist_ai_rmf_elements.json")
KSA_DIR = os.path.join(REPO_ROOT, "ksas")
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "nist_ai_rmf_candidates.json")
...
def main():
    print("Loading NIST AI RMF elements...", flush=True)
    ...
    print("Encoding NIST AI RMF elements (competency_implication)...", flush=True)
    ...
    print(f"  {len(candidates)} elements x {len(ksas)} KSAs = "
