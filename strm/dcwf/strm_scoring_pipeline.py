#!/usr/bin/env python3
"""
WIDAI STRM-003 Strength of Relationship Scoring Pipeline
=========================================================
Multi-method scoring for DCWF v5.1 KSAT → WIDAI KSA mappings.
...
Methodology identical to STRM-001-ONET and STRM-002-NICE.
Bi-encoder candidate identification used as preprocessing to establish
DCWF→WIDAI pairings (2,945 × 497 = 1.46M potential pairs), then full
...
DCWF_ELEMENTS_FILE = os.path.join(REPO_ROOT, "sources/dcwf/dcwf_elements.json")
...
CANDIDATES_FILE = os.path.join(REPO_ROOT, "sources/dcwf/dcwf_ksat_candidates.json")
...
# Cybersecurity-specific out-of-scope terms (same approach as NICE STRM)
CYBER_OOS_TERMS = {
    'malware', 'exploit', 'vulnerability', 'penetration', 'intrusion',
    'forensic', 'forensics', 'comsec', 'sigint', 'sigdev', 'elint',
    'humint', 'geoint', 'masint', 'imint', 'osint', 'dnea',
    'offensive', 'attack', 'weapon', 'warfighting', 'combatant',
    'kinetic', 'munition', 'detonate', 'detonation', 'jamming',
    'spoofing', 'rootkit', 'trojan', 'botnet', 'phishing',
    'ransomware', 'backdoor', 'exfiltrate', 'exfiltration',
    'digital forensics', 'network exploitation', 'cyber operations',
    'targeting', 'target development', 'target network',
    'red team', 'blue team', 'adversary', 'threat actor',
    'counterintelligence', 'intelligence collection', 'intelligence analysis',
    'collection management', 'collection requirements',
...
# ── Load Data ──
def load_dcwf_elements():
    """Load DCWF KSAT elements from extracted JSON."""
    with open(DCWF_ELEMENTS_FILE) as f:
        data = json.load(f)
    return data["elements"], data["work_roles"]
...
def classify_element(desc, cosine_score):
    """Classify DCWF element as in-scope or out-of-scope for WIDAI."""
    desc_lower = desc.lower()
    desc_tokens = tokenize(desc)

    # Check for strong cyber OOS signals
    oos_hits = sum(1 for t in CYBER_OOS_TERMS if t in desc_lower)
    # Check for data/AI in-scope signals
    inscope_hits = sum(1 for t in DATA_AI_TERMS if t in desc_lower)

    # Strong out-of-scope: many cyber terms, few data/AI terms, low cosine
...
def sig_sts(sts_raw, strength, ta, fde_text, ksa_text):
    ...
    if ta["fde_only"] and ta["ksa_only"]:
        parts.append(
            f"DCWF-only terms [{', '.join(ta['fde_only'][:6])}] "
...
def sig_bertscore(p, r, f1, ta):
    ...
    if abs(p - r) < 0.02:
        parts.append(
            f"Balanced P/R — the {fw}-word DCWF element and "
            f"{kw}-word WIDAI KSA cover each other's token-level "
            f"semantic content symmetrically."
        )
    elif p > r:
        parts.append(
            f"P exceeds R by {p - r:.3f} — the DCWF element's {fw} words "
            f"are well-captured by the WIDAI KSA's {kw} words, "
            f"but the KSA contains additional specificity "
            f"not reflected in the DCWF element."
        )
    else:
        parts.append(
            f"R exceeds P by {r - p:.3f} — the WIDAI KSA's {kw} words "
            f"are well-captured by the DCWF element's {fw} words, "
            f"but the DCWF element contains broader scope "
...
def main():
    # ── Load data ──
    print("Loading DCWF elements and WIDAI KSAs...", flush=True)
    dcwf_elements, dcwf_roles = load_dcwf_elements()
    widai_ksas = load_widai_ksas()
    print(f"  DCWF: {len(dcwf_elements)} KSAT elements across {len(dcwf_roles)} work roles", flush=True)
    ...
    # ── Phase 1: Bi-encoder candidate identification ──
    print("Phase 1: Bi-encoder candidate identification...", flush=True)
    print(f"  Encoding {len(widai_ksas)} WIDAI KSAs...", flush=True)
    ksa_texts = [k["statement"] for k in widai_ksas]
    ksa_embeddings = bienc_model.encode(ksa_texts, convert_to_tensor=True, show_progress_bar=True)

    print(f"  Encoding {len(dcwf_elements)} DCWF elements...", flush=True)
    dcwf_texts = [e["description"] for e in dcwf_elements]
    dcwf_embeddings = bienc_model.encode(dcwf_texts, convert_to_tensor=True, show_progress_bar=True)

    print("  Computing cosine similarity matrix...", flush=True)
    cos_scores = util.cos_sim(dcwf_embeddings, ksa_embeddings)

    # For each DCWF element, get top-5 WIDAI KSA candidates
    candidates = {}
    for i, elem in enumerate(dcwf_elements):
        scores = cos_scores[i].cpu().numpy()
        top_indices = np.argsort(scores)[::-1][:5]
        candidates[elem["dcwf_id"]] = {
            "dcwf_id": elem["dcwf_id"],
            "description": elem["description"],
            "element_type": elem["element_type"],
            "top_candidates": [
                {
                    "rank": r + 1,
                    "ksa_id": widai_ksas[idx]["id"],
                    "domain": widai_ksas[idx]["domain"],
                    "cosine": round(float(scores[idx]), 4),
                    "statement": widai_ksas[idx]["statement"]
                }
    ...
    mappings = []
    for elem in dcwf_elements:
        dcwf_id = elem["dcwf_id"]
        desc = elem["description"]
        etype = elem["element_type"].lower()
        if etype == "ksa":
            etype = "ksa"  # DCWF uses combined KSA type

        cand = candidates[dcwf_id]
        best = cand["top_candidates"][0]
        cosine = best["cosine"]
        ...
        if relationship == "No relationship":
            mappings.append({
                ...
                "ref_domain": None,
                "rationale": "Semantic",
                "notes": f"DCWF {etype} classified as out of WIDAI scope. "
                         f"Best candidate: {best['ksa_id']} ({best['domain']}, cosine: {cosine:.3f}). "
                         f"Insufficient semantic overlap with data/AI workforce competencies.",
                "strength": 0
            })
        else:
            secondaries = [
                f"{c['ksa_id']} ({c['domain']}, cosine: {c['cosine']:.3f})"
                for c in cand["top_candidates"][1:3]
            ]
            mappings.append({
                "fde_id": dcwf_id,
                "fde_type": etype,
                ...
                "ref_domain": best["domain"],
                "rationale": "Semantic",
                "notes": f"DCWF {etype} {relationship.lower()} WIDAI {best['domain']} domain. "
    ...
    strm_data = {
        "strm_id": "STRM-003-DCWF",
        "framework": "DCWF v5.1",
        "framework_source": "DoD Cyber Workforce Framework Work Role Tool v5.1 (2025-07-25)",
        "widai_version": "0.5.2",
